function [sys,x0,str,ts] = magnesy(t, x, u, flag, ai,di,xi,yi)
    switch flag,
    case 0
    ...
    sys = [];
    case { 1 ,3, 4, 9 }
    sys = liczSuma(u,ai,di,xi,yi); % Unused flags
    otherwise
...
function sys=liczSuma(u,ai,di,xi,yi)
x=u(1);
y=u(2);
xSum=sum(ai.*((xi-x)./((sqrt(((xi-x).^2)+((yi-y).^2)+(di.^2))).^3)));
ySum=sum(ai.*((yi-y)./((sqrt(((xi-x).^2)+((yi-y).^2)+(di.^2))).^3)));
sys=[xSum ySum];
end